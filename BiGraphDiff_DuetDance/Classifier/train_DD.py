    ...
    for epoch in range(opt.epochs):
        ...
        for i in range(nb_batch):
            ...
            if opt.device == 0:
                ...
                #dist=dist.cuda()
            src_mask, trg_mask = create_masks(src, None, opt)

            preds = model(src, src_mask,opt.is_test)

            opt.optimizer.zero_grad()

            trgg = torch.max(trg_input,1)[1]
            #predd = torch.max(preds,1)[1]
            #predss = preds.cpu().detach().numpy()
            #loss = F.mse_loss(preds,trg,reduction='sum')
            preds = preds[0]

            loss = F.cross_entropy(preds, trgg.long(),reduction='mean')
            ...
            total_loss += loss.item()+total_loss_eval
            ...
            if (i + 1) % opt.printevery == 0:
                 ...
                 avg_loss = loss.item()
                 if opt.floyd is False:
                    print("   %dm: epoch %d [%s%s]  %d%%  loss = %.3f  loss_train = %.3f  loss_eval = %.3f" %\
                    ((time.time() - start)//60, epoch + 1, "".join('#'*(p//5)), "".join(' '*(20-(p//5))), p, avg_loss, loss, total_loss_eval), end='\r')
                 else:
                    print("   %dm: epoch %d [%s%s]  %d%%  loss = %.3f  loss_train = %.3f  loss_eval = %.3f" %\
                    ((time.time() - start)//60, epoch + 1, "".join('#'*(p//5)), "".join(' '*(20-(p//5))), p, avg_loss,loss, total_loss_eval))
        ...
        print("%dm: epoch %d [%s%s]  %d%%  loss = %.3f\nepoch %d complete, loss = %.03f  loss_train = %.3f   loss_eval = %.3f" %\
        ((time.time() - start)//60, epoch + 1, "".join('#'*(100//5)), "".join(' '*(20-(100//5))), 100, avg_loss, epoch + 1, avg_loss, total_loss_train/nb_batch, total_loss_eval))
        total_loss = 0
        if epoch%10==0:
            dst= '/temp_save'
            print("saving weights to " + dst + "/...")
            torch.save(model.state_dict(), cwd+f'{dst}/model_weights_'+str(epoch))
...
def main():
    ...
    parser.add_argument('-data_dir', type=str,default='data/DD_short')
    parser.add_argument('-train_file', type=str,default='data/DD_short/Train.txt')
    ...
    parser.add_argument('-max_len', type=int, default=214) # ajouté pour test, à intégrer dans le code
    ...
    opt.trg_pad = 1
    opt.data_input,opt.data_output,opt.max_frames = Load_a2m_DD.Load_Data(opt.train_file,opt.data_dir,opt.nb_joints,opt.nb_classes)
    model = get_model(opt)
